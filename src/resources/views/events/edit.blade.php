...
<head>
    ...
    <title>Faily - Edytuj wydarzenie</title>
...
<div class="container py-5 text-white">
    <div class="row mb-4">
        ...
        <div class="col-md-4 text-md-end">
            <a href="{{ route('events.show', $event) }}" class="btn text-white border-dark" style="background: linear-gradient(135deg, #5a00a0 0%, #7f00d4 100%);">Powrót do wydarzenia</a>
        </div>
    </div>
    ...
    <div class="card border-dark shadow-sm custom-card-bg mb-4">
        <div class="card-header text-white" style="background: linear-gradient(45deg, #5a4e82 0%, #3a6b8a 100%);" >
            <h4 class="mb-0">Formularz edycji</h4>
        </div>
        <div class="card-body text-white border-black" style="background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%);">
            <form action="{{ route('events.update', $event) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mt-4">
                    <button type="submit" class="btn text-white" style="background: linear-gradient(135deg, #5a00a0 0%, #7f00d4 100%);">Zapisz zmiany</button>
    ...
    @if($event->photos->count() > 0)
        <div class="card border-0 shadow-lg rounded-4 custom-card-bg mb-5 overflow-hidden">
            <div class="card-header text-white" style="background: linear-gradient(45deg, #5a4e82 0%, #3a6b8a 100%);">
                <h4 class="mb-0">Istniejące zdjęcia</h4>
            </div>
                    ...
                    @foreach($event->photos as $photo)
                        <div class="col-md-4">
                            <div class="card h-100 border-0 shadow-sm bg-dark rounded-4 overflow-hidden">
                                <img src="{{ asset('storage/' . $photo->path) }}" class="card-img-top rounded-top" alt="Zdjęcie wydarzenia" style="object-fit: cover; height: 250px;">
                                <div class="card-body text-center">
                                    <form action="{{ route('photos.destroy', $photo) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger btn-sm rounded-pill mt-2" onclick="return confirm('Czy na pewno chcesz usunąć to zdjęcie?')">
                                            Usuń zdjęcie
                                        </button>
                                    </form>
                                </div>
