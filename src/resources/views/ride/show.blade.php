    ...
    <div class="container mt-5 text-color_2">
        <div class="row mb-4">
            ...
            <div class="col-md-4 text-md-end">
                <a href="{{ route('events.show', $ride->event) }}" class="btn btn-gradient text-color_2">{{ __('messages.ridesshow.backToEvent') }}</a>
                @if(auth()->id() === $ride->driver_id)
                    <a href="{{ route('rides.edit', $ride) }}" class="btn btn-gradient text-color_2 ms-2">{{ __('messages.ridesshow.editRide') }}</a>
                @endif
            </div>
        </div>
                        ...
                        @if(auth()->id() !== $ride->driver_id && $availableSeats > 0)
                            <div class="d-grid gap-2">
                                <form action="{{ route('rides.request', $ride) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="btn text-color btn-gradient">{{ __('messages.ridesshow.requestSeat') }}</button>
                                </form>
                            </div>
            ...
            <div class="col-md-4">
                <div class="card custom-card-bg text-color shadow-sm">
                    ...
                    <div class="card-body">
                        ...
                        <p><strong>{{ __('messages.ridesshow.eventDate') }}</strong> {{ \Carbon\Carbon::parse($ride->event->date)->format('d.m.Y H:i') }}</p>
                        <p><strong>{{ __('messages.ridesshow.eventLocation') }}</strong> {{ $ride->event->location_name }}</p>
